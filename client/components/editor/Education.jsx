...
import { getEducationFields } from "@/data/editor";
...
export default function EducationSection({template="Simple",education, addEducation, updateEducation, removeEducation}) {
  // const { education, addEducation, updateEducation, removeEducation } = useProfileStore()
  const [isOpen, setIsOpen] = useState(false);
  const [openIndex, setOpenIndex] = useState(null);
  if(education== null || education.length ===0){
    return (<NoData category="Education" handleClick={()=>{setIsOpen(true);setOpenIndex(education.length);addEducation()}}/>)
  }
  return (
    <div className="editor mx-2">
        ...
        <div key={index} className="mb-4 border-[1px] border-halfwhite rounded-xl hover:border-primary smooth">
            ...
            <div className="flex items-center justify-between">
              ...
              >
                {isOpen && openIndex === index ? (
                  ...
                  <Trash2 color="red" size="20" onClick={()=>removeEducation(index)} className="hover:scale-125 smooth"/>
                )}
              </button>
            </div>
            {isOpen && openIndex === index && (
              getEducationFields(education, updateEducation, index).map((row) => (
                <div
                  key={row.row}
                  className={`${
                    row.fields[0].type === "textarea" ? "" : "grid grid-cols-2"
                  }`}
                >
          ...
          {isOpen && openIndex === index && (
            <div className="border-t-[#bababa] border-[1px] p-4 flex justify-between">
              <button
                onClick={() => {setIsOpen((cur)=>!cur);setOpenIndex(null);removeEducation(index)}}
      ...
      <Button onClick={()=>{setIsOpen(true);setOpenIndex(education.length);addEducation()}} className="add-button w-fit">
