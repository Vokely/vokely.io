...
import { getSimpleTemplate } from "@/data/templates/simple";
...
export const renderNewSimple = (pages, currentPage) => {
  if (pages.length === 0) return null;
  
  return (
    <div>
      {pages[currentPage].map((item, index) => {
        const template = getSimpleTemplate(item);
        if (!template) return null;
        
        return (
          <React.Fragment key={`${index}`} className="border-test">
...
export default function NewSimple({ data, pages, setPages, currentPage, setCurrentPage, template }) {
    ...
    if (containerRef.current) {
      ...
      sections.forEach(section => {
        if (section.items && section.items.length > 0) {
          ...
          const headingTemplate = getSimpleTemplate(headingItem);
          ...
          section.items.forEach(item => {
            if (section.name === "experience") {
              ...
              const expHeadingTemplate = getSimpleTemplate(expHeadingItem);
              ...
              if (Array.isArray(item.description)) {
                item.description.forEach(bulletText => {
                  ...
                  const bulletTemplate = getSimpleTemplate(bulletItem);
              ...
              const projHeadingTemplate = getSimpleTemplate(projHeadingItem);
              ...
              const sectionTemplate = getSimpleTemplate(sectionItem);
  ...
  return (
    <div className="flex flex-col items-center justify-around hide-scrollbar h-full">
      ...
      <div
        ref={containerRef}
        className={`resume-template-2 p-4 box-border h-[1100px] w-[790px] bg-white shadow-lg rounded-lg pb-10 absolute top-[-9999px] opacity-100`}
      >
          {renderNewSimple(pages,currentPage)}
      </div>
      <PageDots
