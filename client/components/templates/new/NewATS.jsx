...
import { getATSTemplate } from "@/data/templates/ats";
...
export const renderNewATS = (pages, currentPage) => {
  if (pages.length === 0) return null;
  
  return (
    <div>
      {pages[currentPage].map((item, index) => {
        const template = getATSTemplate(item);
...
export default function NewATS({ data, pages, setPages, currentPage, setCurrentPage, template, isCanvasGenerating}) {
  const { personalInfo, skills, experience, education, projects, achievements, socialLinks, certifications } = data;
    ...
    if (containerRef.current) {
      ...
      sections.forEach(section => {
        if (section.items && section.items.length > 0) {
          ...
          const headingTemplate = getATSTemplate(headingItem);
          ...
          section.items.forEach(item => {
            if (section.name === "experience") {
              ...
              const expHeadingTemplate = getATSTemplate(expHeadingItem);
              ...
              if (Array.isArray(item.description)) {
                item.description.forEach(bulletText => {
                  ...
                  const bulletTemplate = getATSTemplate(bulletItem);
              ...
              const projHeadingTemplate = getATSTemplate(projHeadingItem);
              ...
              const sectionTemplate = getATSTemplate(sectionItem);
  ...
  return (
    ...
    <div className="relative resume-container h-[70vh] w-[80vw] lap:h-[90%] lap:w-[80%] lap:w-[70%]"></div>
        <div
           ref={containerRef}
           className={`resume-template-2 p-4 box-border h-[1100px] w-[796px] bg-white shadow-lg rounded-lg absolute top-[-9999px] opacity-100`}
         >
          {renderNewATS(pages,currentPage)}
        </div>
        <PageDots
